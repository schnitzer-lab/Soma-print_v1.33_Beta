...
% Check if plot_option is 1, which signifies plotting is desired.
    if plot_option==3;  

    % Before attempting to plot, ensure that we can access the axes handles.
    % If the handles are not present, or the function was called from the command line,
    % we won't be able to plot within the GUI context.
    if isempty(app.inspectAxes) || length(app.inspectAxes) < 6
        fprintf('- Warning: Could not access axes handles for GUI plotting. Skipping plot generation.\n');
        return;
    end

    % If the inputs are cell maps, proceed with the first type of plot.
    
    if length(size(map1))==3; 

        % [Existing computation logic remains the same]
        [peak1,lx1,ly1]=Somaprint_ComputePeak(map1(:,:,1:round(quantile(1:size(map1,3),0.1))));
        [peak2,lx2,ly2]=Somaprint_ComputePeak(map2(:,:,1:round(quantile(1:size(map2,3),0.2))));
        fprintf('- Resizing map, ')
        map1=ReplotMap(map1,mean(lx2(lx2>0))/mean(lx1(lx1>0)));
        ...
        fprintf('- Plotting ...')
        % Define a custom colormap for the first panel.
        mymap=[0 0 0;.5 .5 .5;0 1 0;1 0 1; 1 1 1];
        plot_thre=0.3;
        % Convert maps to binary for cleaner overlay visualization.
        map1=double(map1>plot_thre); map2=double(map2>plot_thre);

        % We use the max-projection overlay to show in-vivo (green) and ex-vivo (magenta) ROIs.
        % The max operation ensures we have a clean logical union of the cell maps.
        % We target the first axes handle directly, avoiding subplot.
        ax_overlay = app.inspectAxes(1);
        cla(ax_overlay); % Clear the axes before drawing to ensure a clean state.
        imagesc(ax_overlay, max(map1(:,:,id_cellout1),[],3)*2 + max(map2(:,:,id_cellout2),[],3)*3);
        colormap(ax_overlay, mymap); % Apply the colormap specifically to these axes.
        set(ax_overlay,'FontSize',fontsize,'Visible','off'); % Set other visual properties.
       
        % For the remaining plots, we apply similar logic: target specific axes, clear them,
        % and then proceed with the plotting as defined by the original logic.

        % Plotting logic for Panel 2
        fprintf(' ... ')
        n=20;plot_map=(map1>0)*(1/n);max_score=max(score);
        for i = 1:length(id_cellout1);
                intensity = 2/n + ((1-2/n) * ((score(i)-cutoff) / max_score));
        ...
        fprintf(' ... ')
        % We can define and apply the colormap only once per panel.
        custom_map_green = zeros(n, 3);
        custom_map_green(1, :) = [0 0 0];
        custom_map_green(2, :) = [0.5, 0.5, 0.5];
        custom_map_green(3:end, 2) = linspace(0.4, 1, n-2)';

        ax_green = app.inspectAxes(2);
        cla(ax_green);
        imagesc(ax_green, max(plot_map,[],3));
        colormap(ax_green, custom_map_green); % Apply the colormap specific to this panel.
        caxis(ax_green, [0 1]); % Lock the color limits.
        set(ax_green,'FontSize',fontsize,'Visible','off');
        
        % Plotting logic for Panel 3
        fprintf(' ... ')
        n=20;plot_map=(map2>0)*(1/n);
        for i = 1:length(id_cellout2);
                intensity = 2/n + ((1-2/n) * ((score(i)-cutoff) / max_score));
        ...
        fprintf(' ... ')
        custom_map_magenta = zeros(n, 3);
        custom_map_magenta(1, :) = [0 0 0];
        custom_map_magenta(2, :) = [0.5, 0.5, 0.5];
        gradient = linspace(0.4, 1, n-2)';
        custom_map_magenta(3:end,1) = gradient;
        custom_map_magenta(3:end,3) = gradient;

        ax_magenta = app.inspectAxes(3);
        cla(ax_magenta);
        imagesc(ax_magenta, max(plot_map,[],3));
        colormap(ax_magenta, custom_map_magenta);
        caxis(ax_magenta, [0 1]);
        set(ax_magenta,'FontSize',fontsize,'Visible','off');

    else
        % Inputs are centroids (X/Y corrected) - plotting logic for Panels 1, 2, and 3
        % Note that when working with centroids, we typically re-use the first three axes handles.
        % We need to clear them before proceeding.
        ax_cent_fixed = app.inspectAxes(1);
        cla(ax_cent_fixed);
        plot(ax_cent_fixed, map1(:,1),map1(:,2),'o','MarkerSize',4,'Color',[0.7 0.7 0.7]);
        hold(ax_cent_fixed, 'on'); % Crucial for direct plotting onto a handle!
        plot(ax_cent_fixed, map1(id_cellout1,1),map1(id_cellout1,2),'o','MarkerSize',4,'Color',[0 0.7 0]);
        set(ax_cent_fixed,'XDir','normal','YDir','reverse','LineWidth',2)
        set(ax_cent_fixed,'XTick',[],'YTick',[]);   
        xl = get(ax_cent_fixed, 'XLim'); yl = get(ax_cent_fixed, 'YLim');
        hold(ax_cent_fixed, 'off'); % Close the 'hold' state.

        ax_cent_moving = app.inspectAxes(2);
        cla(ax_cent_moving);
        plot(ax_cent_moving, map1(id_cellout1,1),map1(id_cellout1,2),'o','MarkerSize',4,'Color',[0 0.7 0]);
        hold(ax_cent_moving, 'on');
        plot(ax_cent_moving, map2(id_cellout2,1),map2(id_cellout2,2),'o','MarkerSize',4,'Color',[.7 0 .7]);
        set(ax_cent_moving,'XDir','normal','YDir','reverse','LineWidth',2)
        set(ax_cent_moving,'XTick',[],'YTick',[]);   
        xlim(ax_cent_moving, xl); ylim(ax_cent_moving, yl);
        hold(ax_cent_moving, 'off');

        % The original code had a duplicate panel 4 when dealing with centroids. We'll skip that.
    end

    % The remaining plots for Panels 4, 5, and 6 are generated using standard histogram and plot commands.
    % We apply the same principle: target the axis handle with cla and hold.

    % Logic for Panel 4: Mixture Gaussian fit
    ax_hist_normal = app.inspectAxes(4);
    cla(ax_hist_normal);
    y_max=ceil(max(b2)*100/10)/10;
    cut_y=0;yl=[cut_y y_max*n_scale];  
    PlotHistogram(ax_hist_normal, b2*n_scale,a,plot_color2nd,cut_y); % Assumes PlotHistogram can take an axes handle.
    hold(ax_hist_normal, 'on');
    PlotHistogram(ax_hist_normal, b1*n_scale,a,plot_color1st,cut_y);
    plot(ax_hist_normal, a_lr,y*n_scale,'Color',plot_color2nd_dark,'LineStyle','--','LineWidth',lw);
    plot(ax_hist_normal, a_lr,pdf(gmmodel,a_lr')*n_scale,'Color',plot_color1st_dark,'LineStyle','--','LineWidth',lw);

    % [Remaining formatting and annotation logic from the original code remains the same, updated for axes handles]
    line(ax_hist_normal, [mean(cutoff),mean(cutoff)],yl,'LineStyle','--','Color',[0.3 .3 0.3],'LineWidth',lw)
    set(ax_hist_normal,'FontSize',fontsize,'box','off','LineWidth',lw);
    set(ax_hist_normal,'XMinorTick','on','YMinorTick','on')
    set(ax_hist_normal,'TickLength',[0.026,1])
    set(ax_hist_normal,'XTick',[0:10:x_max]);
    set(ax_hist_normal,'YTick',[0:0.1:y_max]);
    ylim(ax_hist_normal, yl);
    xlim(ax_hist_normal, [0 x_max]);
    hold(ax_hist_normal, 'off');
    fprintf(' ... \n')

    % Logic for Panel 5: Individual cell scatter
    ax_scatter = app.inspectAxes(6); % We use the last axes handle.
    cla(ax_scatter);
    plot(ax_scatter, score,secondbest,'.','Color',[.5 .5 .5]);
    hold(ax_scatter, 'on');
    plot(ax_scatter, score(1:length(id_cellout1)),secondbest(1:length(id_cellout1)),'.','Color',(plot_color1st_dark+plot_color1st)/2);yl=get(gca,'YLim');
    yl=yl*1.2;
    line(ax_scatter, [mean(cutoff),mean(cutoff)],yl,'LineStyle','--','Color',[.3 .3 0.3],'LineWidth',lw)
    line(ax_scatter, yl,yl,'LineStyle','--','Color',[.6 .6 .6],'LineWidth',lw)
    ylim(ax_scatter, yl);
    set(ax_scatter,'FontSize',fontsize,'box','off','LineWidth',lw);
    hold(ax_scatter, 'off');

    % Logic for Panel 6: Log scale fit (replaces the original duplicate panel 5)
    ax_hist_log = app.inspectAxes(5); 
    cla(ax_hist_log);
    PlotHistogram(ax_hist_log, b2*n_scale,a,plot_color2nd,0);
    hold(ax_hist_log, 'on');
    PlotHistogram(ax_hist_log, b1*n_scale,a,plot_color1st,0);
    plot(ax_hist_log, a_lr,y*n_scale,'Color',plot_color2nd_dark,'LineStyle','--','LineWidth',lw);  
    plot(ax_hist_log, a_lr,pdf(gmmodel,a_lr')*n_scale,'Color',plot_color1st_dark,'LineStyle','--','LineWidth',lw)
    set(ax_hist_log,'YScale','log');    
    line(ax_hist_log, [mean(cutoff),mean(cutoff)],[eps,100],'LineStyle','--','Color',[0.3 .3 0.3],'LineWidth',lw)
    set(ax_hist_log,'FontSize',fontsize,'box','off','LineWidth',lw);
    ylim(ax_hist_log, [0.002 1]*n_scale);
    set(ax_hist_log,'XMinorTick','on','YMinorTick','on')
    set(ax_hist_log,'TickLength',[0.05,1])
    set(ax_hist_log,'XTick',[0:10: x_max]);     
    xlim(ax_hist_log, [0 x_max]);
    hold(ax_hist_log, 'off');

    % Finally, clear the annotation logic from the original code.
    % In a GUI, text summaries should be updated via labels, not annotation boxes that may get clipped.
