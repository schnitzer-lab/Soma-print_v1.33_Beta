function Somaprint_PlotBoundaryCell(map1,image1,id_map1,map2,image2,id_map2,score,n_select,input_id_cell1,input_id_cell2,n_vec,p_sum)

% input_id_cell1, input_id_cell2: anchor cells from previous round

[h1,w1,n1]=size(map1);[temp,temp,n2]=size(map2);
l_buffer=round(size(image2,1)/100*6);
...
n_step=3;
idx=n_select-n_step*3:n_step:n_select+n_step*4;
id_map1_plot=id_map1(idx);
id_map2_plot=id_map2(idx);
...
%imagesc(normimage(image1((center1(id_cell1,1)-l_buffer):(center1(id_cell1,1)+l_buffer),(center1(id_cell1,2)-l_buffer):(center1(id_cell1,2)+l_buffer)),0));colormap('bone');   
...
%imagesc(normimage(image2((center2(id_cell2,1)-l_buffer):(center2(id_cell2,1)+l_buffer),(center2(id_cell2,2)-l_buffer):(center2(id_cell2,2)+l_buffer)),0));colormap('bone');
